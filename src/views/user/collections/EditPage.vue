    ...
    <ion-page>
        <ion-header :translucent="true">
            <ion-toolbar>
                ...
                <ion-title>{{ $t('headers.collections.edit') }}</ion-title>
            ...
            <div class="flex-box">
                <ion-card class="input-box">
                    <ion-item>
                        ...
                        <ion-input v-model="name" type="text" maxlength="40"
                            :placeholder="$t('placeholders.collection.add.name')">
                        </ion-input>
                    ...
                    <ion-item>
                        ...
                        <ion-select v-model="currency" interface="action-sheet"
                            :placeholder="$t('placeholders.collection.add.currency')">
                            <ion-select-option v-for="s in currencies" :key="s.currency" :value="s.abbreviation"
                                v-html="s.symbol+ ' - ' +s.currency"></ion-select-option>
                        </ion-select>

                    </ion-item>
                    <ion-button v-on:click="modifyCollection()" expand="block" ref="submitButton">{{$t('buttons.modify')}}</ion-button>
                </ion-card>
            </div>
        </ion-content>
    </ion-page>
...
export default defineComponent({
    ...
    mounted(){
        this.axios.get('/collections/'+this.$route.params.collection_id).then(response => {
            if (!response.data.data) return;
            this.collectionData = response.data.data;
            this.name = this.collectionData.name;
            this.currency = this.collectionData.currency;
        }).catch(error => {
            console.log(error);
        })
    },
    methods: {
        modifyCollection() {
            ...
            this.axios.put('/collections/'+this.collectionData.id, { name: this.name, currency: this.currency })
                .then(response => {
